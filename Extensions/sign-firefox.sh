#!/usr/bin/env bash
# Submit the packaged Firefox extension to its public Mozilla Add-ons listing.
set -euo pipefail
...
npm exec --yes --package=web-ext@10.6.0 -- web-ext sign \
  --no-config-discovery --source-dir "$SOURCE_DIR" \
  --artifacts-dir "$SIGNED_DIR" --channel listed \
  --timeout 300000 --approval-timeout 900000

shopt -s nullglob
SIGNED_PACKAGES=("$SIGNED_DIR"/*.xpi)
if [[ ${#SIGNED_PACKAGES[@]} -ne 1 ]]; then
  echo "Mozilla has not returned a signed XPI. Check the public submission in the AMO Developer Hub." >&2
  exit 1
fi
...
echo "Check the listing status in the AMO Developer Hub before announcing publication."

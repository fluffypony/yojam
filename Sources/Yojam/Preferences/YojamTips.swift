...
struct SetDefaultBrowserTip: Tip {
    ...
    var rules: [Tips.Rule] {
        [#Rule(Self.$hasSetDefault) { $0 == false }]
    }
...
struct ActivationModeTip: Tip {
    ...
    var rules: [Tips.Rule] {
        [#Rule(Self.$hasChangedMode) { $0 == false }]
    }
...
struct BrowserOrderTip: Tip {
    ...
    var rules: [Tips.Rule] {
        [#Rule(Self.$hasReordered) { $0 == false }]
    }
...
struct URLTesterTip: Tip {
    ...
    var rules: [Tips.Rule] {
        [#Rule(Self.$hasTestedURL) { $0 == false }]
    }
...
struct CustomLaunchArgsTip: Tip {
    ...
    var rules: [Tips.Rule] {
        [#Rule(Self.$hasEditedArgs) { $0 == false }]
    }

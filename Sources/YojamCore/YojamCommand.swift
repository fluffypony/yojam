...
public enum YojamCommand: Sendable {
    ...
    public static func buildRoute(
        ...
        // R6: URLQueryItem encodes most characters but not `+`, which round-trips
        // to space. Pre-encode only `+` before handing to URLQueryItem.
        let targetString = target.absoluteString.replacingOccurrences(of: "+", with: "%2B")
        var items: [URLQueryItem] = [
            URLQueryItem(name: "url", value: targetString)
        ]
